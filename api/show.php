<?php
  ...
  $stmt=$pdo->prepare("select * from qsos where id=:id AND station=:cs order by `date` DESC,time DESC;");
  $stmt->bindValue(":cs",$obj["cs"],PDO::PARAM_STR);
  $stmt->bindValue(":id",$obj["qso"],PDO::PARAM_INT);
  if(!$stmt->execute()){
    http_response_code(400);
